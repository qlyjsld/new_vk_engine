...
{
    create_buffer(pad_uniform_buffer_size(sizeof(glm::vec2)),
                  VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                  VMA_ALLOCATION_CREATE_HOST_ACCESS_RANDOM_BIT, &extent_buffer);
...

    {
        ...
        write_set =
            vk_boiler::write_descriptor_set(&descriptor_buffer_info, pcomp_set, 1,
                                            VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC);

        vkUpdateDescriptorSets(_device, 1, &write_set, 0, nullptr);
    }

    load_shader_module("../shaders/perlin+worley.comp.spv", &pcomp);
    ...
    pcomp_pipeline_builder.build_layout(_device, layouts, push_constants, &pcomp_layout);
    pcomp_pipeline_builder.build_comp(_device, &pcomp_layout, &pcomp_pipeline);
}
